...
from game.constants import (
    TEXT_COLOR, MUTED_TEXT, ACCENT, P1_COLOR, P2_COLOR, WIN_COLOR,
    DIFFICULTY_GRID, BOARD_SIZE, LEFT_BOARD_X, RIGHT_BOARD_X, BOARD_Y,
    WINDOW_WIDTH, WINDOW_HEIGHT, BTN_COLOR, BTN_HOVER, BTN_ACTIVE
)
...
class GameScreen:
    ...
    def __init__(
        ...
        self._font_huge  = pygame.font.SysFont("Georgia", 54, bold=True)
        self._font_big   = pygame.font.SysFont("Georgia", 38, bold=True)
        self._font_hud   = pygame.font.SysFont("Georgia", 22, bold=True)
        self._font_small = pygame.font.SysFont("Georgia", 16)
        ...
        self._btn_back = Button(
            (16, H - btn_h - 14, btn_w, btn_h), "◀  B A C K", self._font_btn,
            color=(max(0, P2_COLOR[0]-20), max(0, P2_COLOR[1]-20), max(0, P2_COLOR[2]-20)), hover_color=P2_COLOR, active_color=(max(0, P2_COLOR[0]-40), max(0, P2_COLOR[1]-40), max(0, P2_COLOR[2]-40)), border_radius=20
        )
        self._btn_restart = Button(
            (16 + btn_w + 10, H - btn_h - 14, btn_w, btn_h), "R E S T A R T", self._font_btn,
            color=BTN_COLOR, hover_color=BTN_HOVER, active_color=BTN_ACTIVE, border_radius=20
        )


        cx = W // 2
        self._btn_next = Button(
            (cx - 115, H - 78, 230, 52), "▶  N E X T", self._font_btn,
            color=BTN_COLOR, hover_color=BTN_HOVER, active_color=BTN_ACTIVE, border_radius=26
        )
        self._btn_menu_final = Button(
            (cx - 115, H - 78, 230, 52), "◀  M A I N   M E N U", self._font_btn,
            color=(max(0, P2_COLOR[0]-20), max(0, P2_COLOR[1]-20), max(0, P2_COLOR[2]-20)), hover_color=P2_COLOR, active_color=(max(0, P2_COLOR[0]-40), max(0, P2_COLOR[1]-40), max(0, P2_COLOR[2]-40)), border_radius=26
        )
    ...
    def handle_event(self, event) -> str | None:
        ...
        # Keyboard WASD for P1, Arrows for P2
    ...
    def _handle_keyboard_move(self, board: Board, key: int) -> None:
        ...
        dr, dc = 0, 0
        if key in (pygame.K_w, pygame.K_UP): dr = 1      # Empty down, tile up
        elif key in (pygame.K_s, pygame.K_DOWN): dr = -1 # Empty up, tile down
        elif key in (pygame.K_a, pygame.K_LEFT): dc = 1  # Empty right, tile left
        elif key in (pygame.K_d, pygame.K_RIGHT): dc = -1# Empty left, tile right
